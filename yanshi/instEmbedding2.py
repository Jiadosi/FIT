...
if __name__ == "__main__":
    modelPath = '../Instruction_Embedding/myModel/x86'
    modelPath2 = '../Instruction_Embedding/myModel/arm'
    ...
    dis0 = np.abs(np.dot(xtest, xmov)/(np.linalg.norm(xtest)*(np.linalg.norm(xmov))))
    #print(np.dot(xtest, xmov))
    print('X86:test~eax,eax---X86:mov~eax,0')
    print(dis0)
    dis00 = np.abs(np.dot(aldr, acmp)/(np.linalg.norm(aldr)*(np.linalg.norm(acmp))))
    # dis00 = np.dot(aldr, acmp)/(np.matmul(aldr, acmp))
    print('ARM:CMP~R0,0---ARM:LDR~R3,[R5+0]')
    print(dis00)
    dis1 = np.abs(np.dot(xtest, acmp)/(np.linalg.norm(xtest)*(np.linalg.norm(acmp))))
    # print(np.dot(xtest, acmp))
    print('X86:test~eax,eax---ARM:CMP~R0,0')
    print(dis1)
    dis1 = np.abs(np.dot(xmov, aldr)/(np.linalg.norm(xmov)*(np.linalg.norm(aldr))))
    print('X86:mov~eax,0---ARM:LDR~R3,[R5+0]')
    print(dis1)
